...
#####################################################################
#
# Next swap current names with fixed names from Ipro_to_NCBI list
#
#####################################################################
'''
Create a script that will look for matching values in two tab delimited files.
compare the third column of augustus.json.complete.tsv to the first column of Ipro_to_NCBI_list.txt
and if there is a match create a line that includes column 1 of augustus.json.complete.tsv, column 4 of 
Ipro_to_NCBI_list.txt and columns 3 to 6 of augustus.json.complete.tsv
output this to a new file called RepairedNames.tsv

'''
# repair_names.py

# Load mappings from Ipro_to_NCBI_list.txt
ipro_to_ncbi = {}
with open("Interpro_to_NCBI_list.txt", "r") as file:
    for line in file:
        parts = line.strip().split("\t")
        if len(parts) >= 2:
            ipro_to_ncbi[parts[0]] = parts[3] # equate IPR with revised name in column 4

# Process augustus.json.complete.tsv and create output
with open("augustus.json.complete.tsv", "r") as infile, open("RepairedNames.tsv", "w") as outfile:
    for line in infile:
        parts = line.strip().split("\t")
        if len(parts) >= 6:
            match_key = parts[2]  # column 3
            if match_key in ipro_to_ncbi:
                output_line = "\t".join([
                    parts[0],                # column 1 from augustus
                    ipro_to_ncbi[match_key], # column 4 from ipro list
                    parts[2],                # column 3 from augustus
                    parts[3],                # column 4 from augustus
                    parts[4],                # column 5 from augustus
                    parts[5]                 # column 6 from augustus
                ]) + "\n"
                outfile.write(output_line)


print(f"Name revision complete. The modified file is saved.")
#####################################################################
...
if __name__ == '__main__':
    input_file = 'RepairedNames.tsv'  # Replace with the actual input file path
...
If they do pass the test, do the following:
...
# Remove spaces before dashes and replace double spaces with single spaces
